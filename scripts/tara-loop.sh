...
target=$("$tsx" scripts/tara-next.ts "$lane" 2>/tmp/tara-next.err || true)
if [ -s /tmp/tara-next.err ]; then
  echo "sira hata: $(head -c 200 /tmp/tara-next.err)"
fi
...
while [ $SECONDS -lt $deadline ]; do
  ...
  if [ -z "$url" ]; then
    echo "Sıradaki adres gelmedi, 15 saniye bekleniyor."
    sleep 15
    target=$("$tsx" scripts/tara-next.ts "$lane" 2>/tmp/tara-next.err || true)
    continue
  fi
  ...
  cevap=$("$tsx" scripts/tara-feed.ts gonder.json 2>/tmp/tara-feed.err || true)
  echo "$cevap" | head -c 400
  echo
  if [ -s /tmp/tara-feed.err ]; then
    echo "feed hata: $(head -c 200 /tmp/tara-feed.err)"
  fi
  ...
  if [ "$extras" != "[]" ] && [ "$extras" != "null" ]; then
    echo "$extras" | jq -c '.[]' | while read -r extra; do
      ...
      "$tsx" scripts/tara-feed.ts gonder.json 2>/tmp/tara-feed.err | head -c 200 || true
      echo
      sleep "$wait_ek"
    done
  fi
